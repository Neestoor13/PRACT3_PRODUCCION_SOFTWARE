...
def test_remove_expense_reduces_total():
    ...
    service.create_expense("Revista", 13, "", date.today())

    expenses_before_elimination = service.list_expenses()
    service.remove_expense(expenses_before_elimination[0].id)
    expenses_after_elimination = service.list_expenses()

    assert len(expenses_after_elimination) == 1
    assert expenses_after_elimination[0].title == "Revista"
...
def test_update_expense_partial_fields():
    ...
    service.update_expense(1, None, 18, None)
